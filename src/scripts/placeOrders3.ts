...
async function main() {
  ...
  const koinosRaffles = new Contract({
  ...
  while (current < TOTAL_NFTS) {
    ...
    for (let i = current; i < current + NFTS_PER_TX; i += 1) {
      ...
      const { result: resultKollection } = await kollection.functions.get_order(
        orderArgs
      );
      const { result: resultKoinosRaffles } =
        await koinosRaffles.functions.get_order(orderArgs);

      if (i % 2 === 0) {
        // even number --> list in Kollection
        ...
        if (!resultKollection) {
          await tx.pushOperation(
            kollection.functions.create_order,
            createOrderArgs
          );
        }

        if (resultKoinosRaffles) {
          await tx.pushOperation(
            koinosRaffles.functions.cancel_order,
            orderArgs
          );
        }
      } else {
        // odd number --> list in Koinos Raffles
        await tx.pushOperation(contract.functions.approve, {
          token_id: tokenId,
          approver_address: contract.getId(),
          to: koinosRaffles.getId(),
        });

        if (!resultKoinosRaffles) {
          await tx.pushOperation(
            koinosRaffles.functions.create_order,
            createOrderArgs
          );
        }

        if (resultKollection) {
          await tx.pushOperation(kollection.functions.cancel_order, orderArgs);
        }
      }
    }
